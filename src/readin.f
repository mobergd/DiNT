...
c read potential interface flag
      ...
      if (potflag.eq.0) then
        IF (my_rank.eq.0) write(6,*)"Using the POTLIB MM-HO-1 interface"
      elseif (potflag.eq.2) then
        IF (my_rank.eq.0) write(6,*)"Using the POTLIB MM-HE-1 interface"
      elseif (potflag.eq.1) then
        IF (my_rank.eq.0) write(6,*)"Using the POTLIB 3-2V interface"
...
c read trajectory conrol flag
     ...
     &  " i.e., steepest descent minimization"
      elseif (tflag(1).eq.2) then
        IF (my_rank.eq.0) write(6,*)"TFLAG(1) = 2:  Temperature ramping"
      elseif (tflag(1).eq.3) then
        IF (my_rank.eq.0) write(6,*)"TFLAG(1) = 3:  Andersen thermostat"
      else
        IF (my_rank.eq.0) write(6,*)"TFLAG(1) != 0-3 is not supported"
        stop
      endif

      if (tflag(2).eq.0) then
      elseif (tflag(2).eq.1) then
       IF (my_rank.eq.0) write(6,*)"TFLAG(2) = 1:  Restart trajectories"
      else
       IF (my_rank.eq.0) write(6,*)"TFLAG(2) != 0 or 1 is not supported"
       stop
      endif
     ...
     &    write(6,*)"TFLAG(3) = 1:  Photoexcited trajectory"
       ...
       IF (my_rank.eq.0) write(6,*)"TFLAG(3) != 0 or 1 is not supported"
       stop
      endif
...
c       convert to 1/a.u.time
     ...
     &      " energy = ",scandth," eV plus the initial potential energy"
          ...
          IF (my_rank.eq.0) write(6,*)"Resulting momenta are not scaled"
        endif
      endif
      if (tflag(1).eq.4) then
        ...
        ENDIF
        if (nsurft.ne.2) then
         IF (my_rank.eq.0) write(6,*)"NSURFT must be 2 for TFLAG(1) = 4"
         stop
        endif
      endif
...
c read info for each molecule
     ...
     &    "ERROR: cant have natom < 1, natom(",im,")=",natom(im)
        ...
        if (nat.gt.mnat) then
         IF (my_rank.eq.0) write(6,*)"ERROR:  nat > mnat, ",nat,">",mnat
         stop
        endif
        IF (my_rank.eq.0) write(6,*)
...
c         compute total mass for this AG
     ...
     &                 0, MPI_COMM_WORLD, ierr)      
        call MPI_BCAST(mmag, nmol, MPI_DOUBLE_PRECISION,
     &                 0, MPI_COMM_WORLD, ierr)      
        call MPI_BCAST(rmindi, nmol, MPI_DOUBLE_PRECISION,
     &                 0, MPI_COMM_WORLD, ierr)      
        call MPI_BCAST(vvdi, nmol, MPI_DOUBLE_PRECISION,
     &                 0, MPI_COMM_WORLD, ierr)      
        call MPI_BCAST(jjdi, nmol, MPI_DOUBLE_PRECISION,
     &                 0, MPI_COMM_WORLD, ierr)      
        call MPI_BCAST(rrad, 1, MPI_DOUBLE_PRECISION,
     &                 0, MPI_COMM_WORLD, ierr)      
        call MPI_BCAST(arrad, 1, MPI_DOUBLE_PRECISION,
...
c          write(6,*)
     ...
     &                   0, MPI_COMM_WORLD, ierr)
        ...
        if (sampwell(im).eq.2) then
          IF (my_rank.eq.0) read(5,*)relwell2(im),emin2(im)
#ifdef MPIFORCES
...
!     &                 0, MPI_COMM_WORLD, ierr)
     ...
     &     "energy of ", samptarg(im)," eV, independent of J"
        ...
        IF (my_rank.eq.0) write(6,*)"Sampled momenta will not be scaled"
...
c         quasiclassical state-selected orientation
     ...
     &      "IORIENT = ",iorient," is not supported"
        ...
        IF (my_rank.eq.0) write(6,*)"Cant have AG < 0"
        stop
      endif
      IF (my_rank.eq.0) write(6,*)
...
c       dissociation
 ...
 200      format(i5,15x,a2,"-",a2,f15.5,a)
     ...
     &                 0, MPI_COMM_WORLD, ierr)
        call MPI_BCAST(t_symb, t_noutcome*2, MPI_CHARACTER,
     &                 0, MPI_COMM_WORLD, ierr)
...
c output flags
      ...
      if (ii.eq.0) then 
        ...
        do i=1,ii
          lwrite(itmp(i)) = .true.
          if (itmp(i).eq.77) write(6,*)
     &      "Unit 77 chosen: Writing special named output files"
        enddo
      endif
      write(6,*)
      ENDIF
#ifdef MPIFORCES
      call MPI_BCAST(lwrite, 99, MPI_LOGICAL,
     &               0, MPI_COMM_WORLD, ierr)
#endif
...
c check for bad input combinations
      ...
      IF (my_rank.eq.0)write(6,*)"POTFLAG = 1 must be used with 3 atoms"
      stop 
      endif
