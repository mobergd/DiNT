...
c to set up each trajectory.
     ...
     &  phop(mnsurf),dmag,ppj(3,mnat),ek2,ejrot,relwell1(im)
      integer im,i,j,k,ii,sampwell(im)
...
C       SCALE MOMENTUM
        if (sampjmin(im).ge.0.d0) then
          IF (my_rank.eq.0) print *,"Removing angular momentum"
#ifdef MPIFORCES
          call MPI_BARRIER(MPI_COMM_WORLD, ierr)
#endif
        ...
        elseif (samptarg(im).le.0.d0) then  ! random J
          IF (my_rank.eq.0) write(6,*)"Choosing J"
...
c WRITE INITIAL ENERGY AND ANGULAR MOMENTUM
      IF (my_rank.eq.0) write(6,*)"Info for this AG only"
#ifdef MPIFORCES
      call MPI_BARRIER(MPI_COMM_WORLD, ierr)
      call MPI_BCAST(ldofrag, 1, MPI_LOGICAL, 0, MPI_COMM_WORLD, ierr)
#endif
