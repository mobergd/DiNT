...
c TEMP CO2, UNCOMMENT TO PUT BACK
...
c     we hit the max steps before we dissociated
        IF (my_rank.eq.0) write(6,*)"Hit the maximum number of steps. ",
     ...
     &      "Atom #",i," ",symbol(i)," is in fragment",ifrag
...
c        if (nfrag(j).eq.1.and.mmfragtot(j).eq.15.9949d0) print *,"REACT"
        ENDIF
      enddo
      IF (my_rank.eq.0) write(6,*)
...
c     &            kerel,etmp2,evib,erotx,xv,xj,theta
      ...
      ENDIF
      do ifrag=1,nprod
        IF (my_rank.eq.0) write(6,*)"Analyzing fragment ",ifrag
        if (nfrag(ifrag).eq.0) then
          IF (my_rank.eq.0) write(6,*)"No atoms in this fragment"
          peag(ifrag)=0.d0
...
c maybe include special analysis for diatoms, but for now use poly analyses
     ...
     & ," (",symbol(itmp2),")"
          ...
          IF (my_rank.eq.0) write(6,*)"Polyatomic (N=>2) fragment"
          nf=nfrag(ifrag)
...
c          if (.false.) then
          ...
          IF (my_rank.eq.0) write(6,*)
     &          "Final PE for this fragment = ",peag(ifrag)*autoev
