...
c LJ parameters from Allen & Tildesly, Computer Simulations of Liquids (1992), p. 21
...
c returns the full Rg-target intermolecular potential and its derivatives
      ...
      parameter(autocmi=219474.63067d0)
      parameter(autoang=0.529177249d0)
      parameter(xkb=3.166829d-6)  ! Boltzmann in hartee/K
      character*2 symb(maxatom)
      dimension at(maxatom)
...
c FROM TABLE 2.1
      ...
      if (m2.eq.21) then      ! He-
        ...
        em=ehe
        if     (m1.eq.1) then !    H
          sig = 0.5d0*(sh+sm)/autoang
          de=4.d0*xkb*dsqrt(eh*em)
        elseif (m1.eq.2) then !    C
          sig = 0.5d0*(sc+sm)/autoang
          de=4.d0*xkb*dsqrt(ec*em)
        elseif (m1.eq.3) then !    N
          sig = 0.5d0*(sn+sm)/autoang
          de=4.d0*xkb*dsqrt(eo*em)
        elseif (m1.eq.4) then !    O
          sig = 0.5d0*(so+sm)/autoang
          de=4.d0*xkb*dsqrt(en*em)
        else
