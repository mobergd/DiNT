...
c local
      ...
      double precision dptmp(3,mnat,mnsurf,mnsurf),zero
      character*8 tname(mnc),tnam,tnameB(mnc)
...
c      tname(1) = "qc.m06"
      mm=0
      do i=1,nat
      if(symb(i).eq."h".or.symb(i).eq."H") mm=mm+1
      if(symb(i).eq."o".or.symb(i).eq."O") mm=mm+16
      enddo
      zero=0.
      if (mm.eq.1) then         ! h
          tname(1) = "qc.h"
      elseif (mm.eq.2) then     ! h2
          tname(1) = "qc.h2"
      elseif (mm.eq.17) then     ! oh
          tname(1) = "qc.oh"
      elseif (mm.eq.18) then     ! h2o
          tname(1) = "qc.h2o"
      elseif (mm.eq.19) then     ! h3o
          tname(1) = "qc.mol"
          tnameB(1) = "qcB.mol"
          tnameC(1) = "qcC.mol"
      else
          print *,"H3O.F PES Cant handle ",mm," atoms"
          stop
      endif
c -------------------
...
c       call Molpro 2006
  101   call dd_m06(symb,tnam,x,y,z,ptmp,gptmp,dptmp,nat,
     &               mnat,ntmp,mnsurf,ifail)
c         handle failures
          if (ifail.eq.0) then
c           everything OK
