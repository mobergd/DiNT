...
c      somax = 404.d0/(2.d0*autocmi)  ! 404/2 cm-1
c      pemd(1,2)=somax
c      pemd(2,1)=somax
c      return

c 1-d expression for SO coupling
      somax = 404.d0/(2.d0*autocmi)  ! 404/2 cm-1
      rcent = 7.0281
      x1 = 0.82652d0
      x2 = 0.078504d0

      do i=1,nat
        if(symb(i).eq."c".or.symb(i).eq."C") icc=i
        if(symb(i).eq."f".or.symb(i).eq."F") iff=i
      enddo
     ...
     &     + (z(icc)-z(iff))**2
      ...
      del = x1+x2*rcom
      arg = (rcom-rcent)/del
      so = somax*0.5d0*(1.d0+dtanh(arg))

c     derivs WRT rcom
      ddel = x2
      darg = (del-(rcom-rcent)*ddel)/del**2
      dso = somax*0.5d0*(1.d0-dtanh(arg)**2)*darg

c     derivs WRT x,y,z
      dx=x(icc)-x(iff)
      dy=y(icc)-y(iff)
      dz=z(icc)-z(iff)
      gpemd(1,icc,1,2) =  dso*dx/rcom
      gpemd(2,icc,1,2) =  dso*dy/rcom
      gpemd(3,icc,1,2) =  dso*dz/rcom
      gpemd(1,iff,1,2) = -dso*dx/rcom
      gpemd(2,iff,1,2) = -dso*dy/rcom
      gpemd(3,iff,1,2) = -dso*dz/rcom
      gpemd(1,icc,2,1) =  dso*dx/rcom
      gpemd(2,icc,2,1) =  dso*dy/rcom
      gpemd(3,icc,2,1) =  dso*dz/rcom
      gpemd(1,iff,2,1) = -dso*dx/rcom
      gpemd(2,iff,2,1) = -dso*dy/rcom
      gpemd(3,iff,2,1) = -dso*dz/rcom
