      ...
      at(i)=0
      if ((symb(i).eq."Cl").or.
     &    (symb(i).eq."cl"))  at(i)=1       ! hydrogen
      if ((symb(i).eq."Ti").or.
     &    (symb(i).eq."ti"))  at(i)=2       ! carbon
      if ((symb(i).eq."Ar").or.
     &    (symb(i).eq."ar").or.
...
c returns the full Rg-target intermolecular potential and its derivatives
      ...
      if (m2.eq.23) then  ! Ar-
        if     (m1.eq.1) then !    Cl
         aa=   7.7396160771507923     
         bb=  0.27773461104159675     
         cc=   9.7000030518509472     
         rrc=   3.0713217566454052     
          aa=(10.d0**aa)
          cutoff=.true.
        elseif (m1.eq.2) then !    Ti
         aa=   6.9997253334147160     
         bb=  0.34315207373271889     
         cc=   7.5345011749626147     
         rrc=   3.1098361156041139     
          aa=(10.d0**aa)
          cutoff=.true.
        else
...
c read template & write QC input file
      ...
      do i=1,100
        ...
        if (string.eq."GEOMETRY") then
          do j=1,nclu
            if (symbol(j).eq."Ti") then
            write(10,fmt='(a8,2x,3f20.10)')"Ti-Ti3+4",x(j),y(j),z(j)
            else
