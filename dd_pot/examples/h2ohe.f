...
c local
      ...
      character*9 tname(mnc),tnam,tnameB(mnc)
...
c           = 2 for Molpro 2006
      ...
      elseif (nat.eq.3) then
          tname(1) = "qc.rrr"
          tnameB(1) = "qcB.rrr"
      elseif (nat.eq.4) then
c          tname(1) = "qc.g09"
          tname(1) = "qc.m06"
          tnameB(1) = "qcB.m06"
      else
          print *,"PES Cant handle ",nat," atoms"
          stop
      endif
c -------------------
...
c           everything OK
            ...
            write(6,*)"QC failure; trying backup"
            ifail=0
            tnam = tnameB(ic)
        call dd_m06(symb,tnam,x,y,z,ptmp,gptmp,dptmp,nat,
     &               mnat,ntmp,mnsurf,ifail)
            if (ifail.ne.0) then
               write(6,*)"QC backup failure; stop"
               stop
            endif
          endif
        else
...
c **********************************************************************
     ...
     &          isurf,jsurf,ifail,ngeo,mgeo,lstr2
...
c     molpro reorders the atoms, and I can't figure out how to make it stop, so...
 ...
 150  read (8,fmt='(a80)',end=170) string
      do i=1,3
        mgeo=1
!        if (nclu.eq.4) mgeo=2
        if (string(i:i+lstr-1).eq.Astr) ngeo=ngeo+1
        if (ngeo.eq.mgeo) then
          read(8,*)
          read(8,*)
          read(8,*)
          do j=1,nclu
