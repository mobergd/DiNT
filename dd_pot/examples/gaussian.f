      subroutine pot(symbol,x,y,z,v,dx,dy,dz,nclu,mnclu)
...
c read template & write QC input file
      open(unit=7,file='qc.g09')  ! template file
      open(unit=10,file='qc.in')   ! temporary input file
      read(7,*)mult                ! multiplicity
      do i=1,100
...
c     init idx_start idx_end
      ...
      open(301,FILE='qc.txt',mode='read')
      write(*,*) " open succeded "
      do i=0,nmol
        write(i_str,'(I0)') i
        write(*,*) "atom group:",i_str
        do while (trim(line).ne.'*start'//trim(i_str))
          read(301,'(A)',iostat=io) line
          if (io.ne.0) then
            write(*,*) "Wrong of qc.txt"
            stop
          endif
        enddo
        idx_start(i) = j + 1
        write(*,*) "idx_start(",i,")=",j+1
        read(301,'(A)',iostat=io) line
  
        do while( trim(line).ne.'*end'//trim(i_str))
          j = j + 1
          qc_buff(j) = trim(line)
          read(301,'(A)',iostat=io) line
          if (io.ne.0) then
