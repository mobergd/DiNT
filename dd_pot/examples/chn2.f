...
cc           retry with numerical gradient in backup template
c            ifail=0
c            do i=1,mnsurf
c            do j=1,mnsurf
c              pemd(i,j)=0.d0
...
c            call dd_m06(symb,tnam,x,y,z,ptmp,gptmp,nat,
c     &                                     mnat,ntmp,mnsurf,ifail)
c            call system_clock(it2)
c            write(83,*)dble(it2-it1)/1.d2,ifail
c            if (ifail.ne.0) then
cc             failed again
c              write(6,*)"QC backup file failure"
c              stop
c            endif
          else
c           can't fix
