...
c **********************************************************************
      ...
      subroutine pot(symb,x,y,z,pema,gpema,dvec,nat,mnat,nsurf,mnsurf)
...
c local
      ...
      character*8 tname(mnc),tnam,tnameB(mnc),tnameC(mnc)
...
c            go to 101
c            endif
c            if (nfail.eq.2) then
c            write(6,*)"QCB failure"
c            ifail=0
c            tnam = tnameC(ic)
c            go to 101
c            endif
c            if (nfail.eq.2) then
c            write(6,*)"QCC failure"
            stop
            endif
          endif
        else
