...
c      do i=1,3
c      do j=1,nx
c      coord(j,i)=coord(j,i)+eps
c      call calcpot(a,coord)
c      coord(j,i)=coord(j,i)-2.d0*eps
c      call calcpot(b,coord)
c      coord(j,i)=coord(j,i)+eps
c      g=(a-b)/(2.d0*eps)
c      gpemd(i,j,1,1)=g
c      enddo
c      enddo
          ...
          zso(i)=z(i)*autoang
          enddo
          call hncoso(xso,yso,zso,eso)

      eps=1.d-5
      do i=1,3
      do j=1,4
      if (i.eq.1) xso(j)=xso(j)+eps
      if (i.eq.2) yso(j)=yso(j)+eps
      if (i.eq.3) zso(j)=zso(j)+eps
      call hncoso(xso,yso,zso,a)
      if (i.eq.1) xso(j)=xso(j)-2.d0*eps
      if (i.eq.2) yso(j)=yso(j)-2.d0*eps
      if (i.eq.3) zso(j)=zso(j)-2.d0*eps
      call hncoso(xso,yso,zso,b)
      if (i.eq.1) xso(j)=xso(j)+eps
      if (i.eq.2) yso(j)=yso(j)+eps
      if (i.eq.3) zso(j)=zso(j)+eps
      g=(a-b)/(2.d0*eps)*autoang/autocmi
      gpemd(i,j,1,2)=g
      gpemd(i,j,2,1)=g
      enddo
      enddo
