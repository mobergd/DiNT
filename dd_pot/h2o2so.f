...
c **********************************************************************
      ...
      implicit none
      double precision autoang,autocmi,autoev
...
c           = 2 for Molpro 2006
      ...
      tname(3) = "qc.so" 
       ...
       itmp=0
       do  i=1,nat
        if (symb(i).eq."h") itmp=itmp+10
        if (symb(i).eq."o") itmp=itmp+1
       enddo
       if (itmp.eq.21) then    ! h2o
       nc=1
       ns(1)=1
       np(1)=1
       tname(1)="qc.h2o"
       elseif (itmp.eq.11) then  ! oh
      nc    = 1
      ns(1) = 1
      np(1) = 1
      tname(1) = "qc.oh"
       elseif (itmp.eq.12) then  ! ho2
      nc    = 1
      ns(1) = 1
      np(1) = 1
      tname(1) = "qc.ho2"
...
c off-diagonal elements (set to zero here)      do i=1,nsurf
      ...
      elseif (itmp.eq.12) then ! ho2
         pemd(2,2)=pemd(1,1)
      elseif (itmp.eq.21) then ! h2o
         pemd(2,2)=pemd(1,1)
      endif

c        print *,pemd(1,1)*autoev
c        print *,pemd(2,2)*autoev
c        print *,pemd(1,2)*autocmi
c        print *,pemd(2,1)*autocmi
...
c off-diagonal elements (set to zero here)
      do i=1,nsurf
      do j=i+1,nsurf
        pemd(i,j)=157./autocmi    ! full cas/6-311++g** gap at b3lyp geoms, energies
        pemd(j,i)=pemd(i,j)
        do k=1,nclu
          gpemd(1,k,i,j)=0.d0
          gpemd(2,k,i,j)=0.d0
