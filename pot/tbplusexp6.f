...
c exp-6 parameters optimized to 3 QCISD(T)/CBS CH4---M cuts (face, edge, vertex)
c     for He: fit is good, but the vdW wells are longer-ranged in the fit than in the q.c.
c     for Ne: fit is good to newer counterpoise corrected CBS
...
c      call aipot(symb2,x2,y2,z2,v2,dvdx2,dvdy2,dvdz2,natom2,maxatom)
      call tbpot(symb2,x2,y2,z2,v2,dvdx2,dvdy2,dvdz2,natom2,maxatom)
      v2=v2+0.666297194   ! zero of E at min of CH4
c      v2=v2+1.14029182   ! zero of E at min of CH3CH3
      call rgexp(symb,x,y,z,v,dvdx,dvdy,dvdz,natom,maxatom)
      ...
      subroutine rgexp(symb,x,y,z,v,dvdx,dvdy,dvdz,natom,maxatom)
...
c returns the full Rg-target intermolecular potential and its derivatives
     ...
     &    (symb(i).eq."KR")) at(i)=24      ! krypton
      ...
      if (m2.eq.21) then      ! He-
c         tinkered with to fit qcisd(t)/av5z
        if     (m1.eq.1) then !    H
          aa=6.040254d0
          bb=0.286639d0
          cc=5.124943d0
...
c       new GA fit to cc qcisd(t)/cbs(adz,atz) CH4+Ne
        if     (m1.eq.1) then !    H
          aa=6.39071017000
          bb=0.26216342700
          cc=5.74940641000
