      ...
      dimension xx(9,3),xxng(9,3)
      character*2 symb(maxatom)
      parameter(autoang=0.529177249d0)
      parameter(autokcal=627.509d0)

      if (natom.ne.1.and.natom.ne.9.and.natom.ne.8) then
        print *,"natom must = 1, 8, or 9 in C2H7aj"
        stop
      endif
...
c numerical gradients
      ...
      do j=1,natom
       xxng(j,i)=xx(j,i)+xst
       call c2h7pot(xxng,up1)
       up1=up1/autokcal*autoang
       xxng(j,i)=xx(j,i)+2.d0*xst
       call c2h7pot(xxng,up2)
       up2=up2/autokcal*autoang
       xxng(j,i)=xx(j,i)-xst
       call c2h7pot(xxng,down1)
       down1=down1/autokcal*autoang
       xxng(j,i)=xx(j,i)-2.d0*xst
       call c2h7pot(xxng,down2)
       down2=down2/autokcal*autoang
       if (i.eq.1) dvdx(j)=(-up2+8.d0*up1-8.d0*down1+down2)/(12.d0*xst)
       if (i.eq.2) dvdy(j)=(-up2+8.d0*up1-8.d0*down1+down2)/(12.d0*xst)
       if (i.eq.3) dvdz(j)=(-up2+8.d0*up1-8.d0*down1+down2)/(12.d0*xst)
       xxng(j,i)=xx(j,i)
      enddo
      enddo
