      ...
      dimension x(mnclu),y(mnclu),z(mnclu) 
      dimension gpemd(3,mnclu,mnsurf,mnsurf),pemd(mnsurf,mnsurf),r(nt,3)
      dimension du11(3,nt),du12(3,nt),du22(3,nt),xx(3,3)
...
c     transform to internals
        ...
        r(1,2)=0.d0
        r(1,3)=0.d0
        r(1,1) = r(1,1) + (x(1)-x(2))**2
        r(1,2) = r(1,2) + (x(2)-x(3))**2
        r(1,3) = r(1,3) + (x(1)-x(3))**2
        r(1,1) = r(1,1) + (y(1)-y(2))**2
        r(1,2) = r(1,2) + (y(2)-y(3))**2
        r(1,3) = r(1,3) + (y(1)-y(3))**2
        r(1,1) = r(1,1) + (z(1)-z(2))**2
        r(1,2) = r(1,2) + (z(2)-z(3))**2
        r(1,3) = r(1,3) + (z(1)-z(3))**2
      do i=1,3
        r(1,i)=dsqrt(r(1,i))
      enddo
      call pot2(r,u11,du11,1,1)
      call pot2(r,u12,du12,1,2)
      call pot2(r,u22,du22,1,3)
      pemd(1,1) = u11
      pemd(1,2) = u12
      pemd(2,1) = u12
      pemd(2,2) = u22
...
c       convert from internals to cartesian
        gpemd(i,1,1,1) = du11(1,1)/r(1,1)*(xx(i,1)-xx(i,2))
     *                  +du11(3,1)/r(1,3)*(xx(i,1)-xx(i,3))
        gpemd(i,2,1,1) = du11(1,1)/r(1,1)*(xx(i,2)-xx(i,1))
     *                  +du11(2,1)/r(1,2)*(xx(i,2)-xx(i,3))
        gpemd(i,3,1,1) = du11(2,1)/r(1,2)*(xx(i,3)-xx(i,2))
     *                  +du11(3,1)/r(1,3)*(xx(i,3)-xx(i,1))
        gpemd(i,1,1,2) = du12(1,1)/r(1,1)*(xx(i,1)-xx(i,2))
     *                  +du12(3,1)/r(1,3)*(xx(i,1)-xx(i,3))
        gpemd(i,2,1,2) = du12(1,1)/r(1,1)*(xx(i,2)-xx(i,1))
     *                  +du12(2,1)/r(1,2)*(xx(i,2)-xx(i,3))
        gpemd(i,3,1,2) = du12(2,1)/r(1,2)*(xx(i,3)-xx(i,2))
     *                  +du12(3,1)/r(1,3)*(xx(i,3)-xx(i,1))
        gpemd(i,1,2,1) = gpemd(i,1,1,2)
        gpemd(i,2,2,1) = gpemd(i,2,1,2)
        gpemd(i,3,2,1) = gpemd(i,3,1,2)
        gpemd(i,1,2,2) = du22(1,1)/r(1,1)*(xx(i,1)-xx(i,2))
     *                  +du22(3,1)/r(1,3)*(xx(i,1)-xx(i,3))
        gpemd(i,2,2,2) = du22(1,1)/r(1,1)*(xx(i,2)-xx(i,1))
     *                  +du22(2,1)/r(1,2)*(xx(i,2)-xx(i,3))
        gpemd(i,3,2,2) = du22(2,1)/r(1,2)*(xx(i,3)-xx(i,2))
     *                  +du22(3,1)/r(1,3)*(xx(i,3)-xx(i,1))
      enddo
...
C
C   System:          MCH
C   Common name:     MCH-SB
C   Functional form: Modified extended LEPS
C   Number of derivatives: 1
C   Number of electronic surfaces: 2
C   Interface: 3-2V
C
C   Reference:  A. W. Jasper and D. G. Truhlar, J. Chem. Phys., submitted (2004).
C
C   Notes:  This PEF is a modification of the MXH SB PEF.  The coupling has
C           been modified to create a conical intersection in the interaction
C           region.  Modification by Ahren Jasper, April 15, 2004.
C
C   Protocol:
...
c ********************************************************************
      ...
      subroutine prepot
        ...
        if (g_myrank .eq. 0) then
          write(6,*)'PREPOT called for MCH (SB)'
          write(6,*)
        endif
...
C
        implicit none
C
        double precision alp, bet, rd0, r20, theta
...
C
     ...
     *ax_), g_gc(g_pmax_)
        ...
        integer g_ehfid
        data alp /3.0d0/, rd0 /3.0d0/, bet /1.0d0/, r20 /-1.3d0/
        double precision ap

c new variables added by AJ
        double precision ang,r3o,r2o,line,g_line(3)

        save
C
        data g_ehfid /0/
        parameter(ap = 0.3017d0)
C
C
        if (g_p_ .gt. g_pmax_) then
          print *, 'Parameter g_p_ is greater than g_pmax_'
...
c ******************************************************************
c Added by AJ 4/15/04 to make MXH into a system with a CI
c angle of the line of zeros in u12 formed WRT r2 = 0 in the (r2,r3) plane
      ang = 45.d0   /180.d0*dacos(-1.d0)
c coordinates of the minimum of the well in u22 (occurs at 180 deg)
      r3o = 3.14d0
      r2o = 2.08d0
      line = dsin(ang)*(r(i,3)-r3o)-dcos(ang)*(r(i,2)-r2o)
      g_line(1) = 0.d0
      g_line(2) = -dcos(ang)
      g_line(3) = dsin(ang)
          ...
          do g_i_ = 1, g_p_
c chain rule
            g_e(g_i_, i) = line * g_e(g_i_, i) + g_line(g_i_)*e(i)
          enddo
          e(i) = e(i) * line
c end new stuff by AJ
c ******************************************************************
