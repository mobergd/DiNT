c AJ found typos & fixed cmplx variables issues 2/26/10
c Rosendo Valero wrote the rest of the CH3X code 2009
c      program ch3f
...
c      parameter(mnat=5)
     ...
     & gtrip3(3,mnat,3,3),sing3b(3,3)
      double precision sing3bis(3,3),gsing3bis(3,mnat,3,3)
     ...
     &singpi2
      ...
      double precision delta,aa,int
      complex*16 lambda(12,12)
     ...
     &gtrip3b(3,mnat),inter(3,mnat)
      ...
      integer itimes,l,m
      complex*16 glambda(3,mnat,12,12),ginterm(3,mnat,12,12),
     &  gomega(3,mnat,12,12)
      double precision plgndr
      double precision thx2(2),thx3(2),thx4(2),thx5(2),thy2(2),thy3(2),
     & thy4(2),thy5(2),thz2(2),thz3(2),thz4(2),thz5(2)
      double precision gth(3,mnat)
      double precision phix2(2),phix3(2),phix4(2),phix5(2),phiy2(2),
     & phiy3(2),phiy4(2),phiy5(2),phiz2(2),phiz3(2),phiz4(2),phiz5(2)
      double precision gphi(3,mnat) 
      double precision singsigth(2),singpixth(2),singpiyth(2),
     & tripsigth(2),trippixth(2),trippiyth(2),thdif
      double precision gsingsigth,gsingpixth,gsingpiyth,gtripsigth,
     & gtrippixth,gtrippiyth
      double precision singsigphi(2),singpixphi(2),singpiyphi(2),
     & tripsigphi(2),trippixphi(2),trippiyphi(2),phidif
      double precision gsingsigphi,gsingpixphi,gsingpiyphi,gtripsigphi,
     &  gtrippixphi,gtrippiyphi    
      double precision gsingsig(3,mnat),gsingpi(3,mnat),
     & gtripsig(3,mnat),gtrippi(3,mnat),gc1(3,mnat),gc2(3,mnat)

      common /pot_coe/ coefF,coefCl,coefBr
      common /SOC_matrix/ SOC

c      open(1,file='molgrad')
...
c      call system("cp qcch3.m06 qc.m06")
...
c      call system("cp qcch4f.m06 qc.m06")

      n3=3
      call dd(symb,x,y,z,sing3,gsing3,nat,mnat,n3,n3)

c      pemd(1,1)=sing3(1,1)
c      pemd(2,2)=sing3(2,2)
c      pemd(3,3)=sing3(3,3)
c      do i=1,3
c       do j=1,nat 
c        gpemd(i,j,1,1) = gsing3(i,j,1,1) 
c        gpemd(i,j,2,2) = gsing3(i,j,2,2) 
c        gpemd(i,j,3,3) = gsing3(i,j,3,3) 
c       enddo
c      enddo
...
c      write(1,*) 'Analytical triplet gradients' 
c      do i=1,mnat
c       write(1,*) "gE1(",i,") = ",gsing3(1,i,1,1),gsing3(2,i,1,1),
...
c      do i=1,mnat
c       write(1,*) ,"gE2(",i,") = ",gsing3(1,i,2,2),gsing3(2,i,2,2),
...
c      do i=1,mnat
c        write(1,*) ,"gE3(",i,") = ",gsing3(1,i,3,3),gsing3(2,i,3,3),
...
C       Calculate numerical derivatives of theta wrt Cartesian coordinates of atoms
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thx2(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)
     &   /rfcm)
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thx3(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)/
     &  rfcm)
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thx4(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)/
     &  rfcm)
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thx5(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)/
     &  rfcm)
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thy2(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)/
     &  rfcm)
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thy3(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)/
     &   rfcm)
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thy4(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)/
     &  rfcm)
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thy5(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)/
     &  rfcm)
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thz2(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)/
     &  rfcm)
        ...
        zh1h2 = zz(3) - z(2)
        xh2h3 = x(4) - x(3)
        yh2h3 = y(4) - y(3)                                ! FIXED THIS AJ
        zh2h3 = z(4) - zz(3)                               ! Fixed this AJ
        rh1h2 = dsqrt(xh1h2**2 + yh1h2**2 + zh1h2**2)
        rh2h3 = dsqrt(xh2h3**2 + yh2h3**2 + zh2h3**2)
        th1 = acos((xh1h2*xh2h3 + yh1h2*yh2h3 + zh1h2*zh2h3)/
     &  (rh1h2*rh2h3))
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thz3(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)/
     &   rfcm)
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thz4(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)/
     &  rfcm)
...
C       Theta is the angle between the vector normal to the plane and the F-CM vector
        ...
        thz5(j) = 360.d0/(2.d0*pi)*acos((nx*xfcm + ny*yfcm + nz*zfcm)/
     &  rfcm)

        enddo

C  Cartesian derivatives of theta

        gth(1,1) = 0.d0
        gth(1,2) = (thx2(2) - thx2(1))/(2.d0*delta)
        gth(1,3) = (thx3(2) - thx3(1))/(2.d0*delta)
        gth(1,4) = (thx4(2) - thx4(1))/(2.d0*delta)
        gth(1,5) = (thx5(2) - thx5(1))/(2.d0*delta)

        gth(2,1) = 0.d0
        gth(2,2) = (thy2(2) - thy2(1))/(2.d0*delta)
        gth(2,3) = (thy3(2) - thy3(1))/(2.d0*delta)
        gth(2,4) = (thy4(2) - thy4(1))/(2.d0*delta)
        gth(2,5) = (thy5(2) - thy5(1))/(2.d0*delta)

        gth(3,1) = 0.d0
        gth(3,2) = (thz2(2) - thz2(1))/(2.d0*delta)
        gth(3,3) = (thz3(2) - thz3(1))/(2.d0*delta)
        gth(3,4) = (thz4(2) - thz4(1))/(2.d0*delta)
        gth(3,5) = (thz5(2) - thz5(1))/(2.d0*delta)
...
C       Final Phi is the average of Phi1, Phi2, Phi3 
        ...
        xcm = (x(2) + x(3) + x(4))/3.d0     ! added recalculation of xcm, etc, and xfcm here AJ
        ycm = (y(2) + y(3) + y(4))/3.d0
        zcm = (z(2) + z(3) + z(4))/3.d0
...
C     Calculate Cartesian derivatives of phi
...
C       Projection of F-CM vector on the plane is n^(rfcm^n)
     ...
     & + zcmh1*zcmh3)/(rcmh1*rcmh3))
      ...
      enddo
        ...
        yh1h2 = yy(3) - y(2)
        zh1h2 = z(3) - z(2)
        xh2h3 = x(4) - x(3)    ! fixed AJ
        yh2h3 = y(4) - yy(3)   ! fixed AJ
        zh2h3 = z(4) - z(3)
        rh1h2 = dsqrt(xh1h2**2 + yh1h2**2 + zh1h2**2)
        rh2h3 = dsqrt(xh2h3**2 + yh2h3**2 + zh2h3**2)
        th1 = acos((xh1h2*xh2h3 + yh1h2*yh2h3 + zh1h2*zh2h3)/
...
C       Projection of F-CM vector on the plane is n^(rfcm^n)
     ...
     & + zcmh1*zcmh3)/(rcmh1*rcmh3))
      ...
      if(abs((phi1+phi3)-angh1cmh3).lt.0.01) then
       ...
       phiz4(j) = (phi1 + phi2 + phi3)/3.d0    ! fixed this
...
C     Derivatives of phi
      ...
      gphi(1,1) = 0.d0
      gphi(1,2) = (phix2(2) - phix2(1))/(2.d0*delta)
      gphi(1,3) = (phix3(2) - phix3(1))/(2.d0*delta)
      gphi(1,4) = (phix4(2) - phix4(1))/(2.d0*delta)
      gphi(1,5) = (phix5(2) - phix5(1))/(2.d0*delta)

      gphi(2,1) = 0.d0
      gphi(2,2) = (phiy2(2) - phiy2(1))/(2.d0*delta)
      gphi(2,3) = (phiy3(2) - phiy3(1))/(2.d0*delta)
      gphi(2,4) = (phiy4(2) - phiy4(1))/(2.d0*delta)
      gphi(2,5) = (phiy5(2) - phiy5(1))/(2.d0*delta)

      gphi(3,1) = 0.d0
      gphi(3,2) = (phiz2(2) - phiz2(1))/(2.d0*delta)
      gphi(3,3) = (phiz3(2) - phiz3(1))/(2.d0*delta)
      gphi(3,4) = (phiz4(2) - phiz4(1))/(2.d0*delta)
      gphi(3,5) = (phiz5(2) - phiz5(1))/(2.d0*delta)
...
c       print *,"sing3(1,1)",sing3(1,1)*autoev
c       print *,"sing3(2,2)",sing3(2,2)*autoev
c       print *,"sing3(3,3)",sing3(3,3)*autoev

c       print *,"ss",singsig*autoev
c       print *,"sp",singpi*autoev
c       print *,"ts",tripsig*autoev
c       print *,"tp",trippi*autoev
...
c       print *,"c1",c1*autoev
c       print *,"c2",c2*autoev
...
c        print *,"ajsigma",ajsigma*autoev
c        print *,"ajpi",ajpi*autoev

        trip3(2,2)= sing3(2,2) - 2.d0*ajpi
        trip3a = sing3(1,1) - 2.d0*ajsigma

c        print *,"trip3(2,2)",trip3(2,2)*autoev
c        print *,"trip3a",trip3a*autoev

c        print *,"trippiy",trippiy*autoev
c        print *,"trippix",trippix*autoev
c        print *,"singpiy",singpiy*autoev
c        print *,"singpix",singpix*autoev

c ROSENDO, sing3b(3,3) diverges when singpiy=singpix
c          this leads to large positive or negative energies
c          for trip3(1,1) and trip3(3,3)
C       RV remove the denominator from sing3b(3,3)
        sing3b(3,3) = sing3(2,2) + (sing3(3,3)-sing3(2,2))*
     &  (trippiy - trippix)

c        print *,"sing3b(3,3)",sing3b(3,3)*autoev
c        print *,"ech3",ech3*autoev
...
c        print *,"trip3(1,1)",trip3(1,1)*autoev
c        print *,"trip3(3,3)",trip3(3,3)*autoev
c        print *,"trip3b",trip3b*autoev

c         stop

C     Calculate derivatives wrt theta,phi
      ...
      do j=1,2
        ...
        do i=1,10
         singsigth(j) = singsigth(j) + Fp(i)*coefF(i)
        enddo

        do i=1,10
         singpixth(j) = singpixth(j) + Fp(i)*coefF(i+10)
        enddo

        do i=1,10
         singpiyth(j) = singpiyth(j) + Fp(i)*coefF(i+20)
        enddo

        do i=1,10
         tripsigth(j) = tripsigth(j) + Fp(i)*coefF(i+30)
        enddo

        do i=1,10
         trippixth(j) = trippixth(j) + Fp(i)*coefF(i+40)
        enddo

        do i=1,10
         trippiyth(j) = trippiyth(j) + Fp(i)*coefF(i+50)
        enddo

        elseif(symb(5).eq."cl") then

        do i=1,10
         singsigth(j) = singsigth(j) + Fp(i)*coefCl(i)
        enddo

        do i=1,10
         singpixth(j) = singpixth(j) + Fp(i)*coefCl(i+10)                         
        enddo

        do i=1,10
         singpiyth(j) = singpiyth(j) + Fp(i)*coefCl(i+20)
        enddo

        do i=1,10
         tripsigth(j) = tripsigth(j) + Fp(i)*coefCl(i+30)
        enddo

        do i=1,10
         trippixth(j) = trippixth(j) + Fp(i)*coefCl(i+40)
        enddo

        do i=1,10
         trippiyth(j) = trippiyth(j) + Fp(i)*coefCl(i+50)
        enddo

        elseif(symb(5).eq."br") then

        do i=1,10
         singsigth(j) = singsigth(j) + Fp(i)*coefBr(i)
        enddo

        do i=1,10
         singpixth(j) = singpixth(j) + Fp(i)*coefBr(i+10)                         
        enddo

        do i=1,10
         singpiyth(j) = singpiyth(j) + Fp(i)*coefBr(i+20)
        enddo

        do i=1,10
         tripsigth(j) = tripsigth(j) + Fp(i)*coefBr(i+30)
        enddo

        do i=1,10
         trippixth(j) = trippixth(j) + Fp(i)*coefBr(i+40)
        enddo

        do i=1,10
         trippiyth(j) = trippiyth(j) + Fp(i)*coefBr(i+50)
        enddo
       ...
       gsingsigth = (singsigth(2) - singsigth(1))/(2.d0*delta) 
       gsingpixth = (singpixth(2) - singpixth(1))/(2.d0*delta)
       gsingpiyth = (singpiyth(2) - singpiyth(1))/(2.d0*delta)
       gtripsigth = (tripsigth(2) - tripsigth(1))/(2.d0*delta)
       gtrippixth = (trippixth(2) - trippixth(1))/(2.d0*delta)
       gtrippiyth = (trippiyth(2) - trippiyth(1))/(2.d0*delta)      
      ...
      do j=1,2
        ...
        phidif = phi + (-1)**j *delta
        z1 = cos(2.0*pi*th/360.0)
        z2 = cos(6.0*pi*phidif/360.0)
        z3 = sin(6.0*pi*phidif/360.0)
        z4 = cos(12.0*pi*phidif/360.0)
        z5 = sin(12.0*pi*phidif/360.0)
        Fp(1) = 1.0
        Fp(2) = plgndr(2,0,z1)
        Fp(3) = plgndr(3,3,z1) * z2
        Fp(4) = plgndr(3,3,z1) * z3
        ...
        do i=1,10
         singsigphi(j) = singsigphi(j) + Fp(i)*coefF(i)
        enddo

        do i=1,10
         singpixphi(j) = singpixphi(j) + Fp(i)*coefF(i+10)
        enddo

        do i=1,10
         singpiyphi(j) = singpiyphi(j) + Fp(i)*coefF(i+20)
        enddo

        do i=1,10
         tripsigphi(j) = tripsigphi(j) + Fp(i)*coefF(i+30)
        enddo

        do i=1,10
         trippixphi(j) = trippixphi(j) + Fp(i)*coefF(i+40)
        enddo

        do i=1,10
         trippiyphi(j) = trippiyphi(j) + Fp(i)*coefF(i+50)
        enddo

        elseif(symb(5).eq."cl") then

        do i=1,10
         singsigphi(j) = singsigphi(j) + Fp(i)*coefCl(i)
        enddo

        do i=1,10
         singpixphi(j) = singpixphi(j) + Fp(i)*coefCl(i+10)                         
        enddo

        do i=1,10
         singpiyphi(j) = singpiyphi(j) + Fp(i)*coefCl(i+20)
        enddo

        do i=1,10
         tripsigphi(j) = tripsigphi(j) + Fp(i)*coefCl(i+30)
        enddo

        do i=1,10
         trippixphi(j) = trippixphi(j) + Fp(i)*coefCl(i+40)
        enddo

        do i=1,10
         trippiyphi(j) = trippiyphi(j) + Fp(i)*coefCl(i+50)
        enddo

        elseif(symb(5).eq."br") then

        do i=1,10
         singsigphi(j) = singsigphi(j) + Fp(i)*coefBr(i)
        enddo

        do i=1,10
         singpixphi(j) = singpixphi(j) + Fp(i)*coefBr(i+10)                         
        enddo

        do i=1,10
         singpiyphi(j) = singpiyphi(j) + Fp(i)*coefBr(i+20)
        enddo

        do i=1,10
         tripsigphi(j) = tripsigphi(j) + Fp(i)*coefBr(i+30)
        enddo

        do i=1,10
         trippixphi(j) = trippixphi(j) + Fp(i)*coefBr(i+40)
        enddo

        do i=1,10
         trippiyphi(j) = trippiyphi(j) + Fp(i)*coefBr(i+50)
        enddo
       ...
       gsingsigphi = (singsigphi(2) - singsigphi(1))/(2.d0*delta) 
       gsingpixphi = (singpixphi(2) - singpixphi(1))/(2.d0*delta)
       gsingpiyphi = (singpiyphi(2) - singpiyphi(1))/(2.d0*delta)
       gtripsigphi = (tripsigphi(2) - tripsigphi(1))/(2.d0*delta)
       gtrippixphi = (trippixphi(2) - trippixphi(1))/(2.d0*delta)
       gtrippiyphi = (trippiyphi(2) - trippiyphi(1))/(2.d0*delta)      


C     Now calculate gradients of c1 and c2
      
      do i=1,3 
       do j=1,5      ! changed this AJ

       gsingsig(i,j) = gsingsigth*gth(i,j) + gsingsigphi*gphi(i,j)

       gsingpi(i,j) = (gsingpixth*gth(i,j) + gsingpiyth*gth(i,j) +
     &  gsingpixphi*gphi(i,j) + gsingpiyphi*gphi(i,j))/2.d0

       gtripsig(i,j) = gtripsigth*gth(i,j) + gtripsigphi*gphi(i,j)

       gtrippi(i,j) = (gtrippixth*gth(i,j) + gtrippiyth*gth(i,j) +
     &  gtrippixphi*gphi(i,j) + gtrippiyphi*gphi(i,j))/2.d0  

       gc1(i,j) = (gsingsig(i,j) + 2.d0*gsingpi(i,j) - 
     &  7.d0*gtripsig(i,j) + 6.d0*gtrippi(i,j))/         ! changed this AJ
     &  (5.d0*(singsig - tripsig)) -
     &  (singsig + 2.d0*singpi - 7.d0*tripsig + 6.d0*trippi)*    
     &  (5.d0*(gsingsig(i,j) - gtripsig(i,j)))/       ! changed this, but this form is equivalent to the last one, I think
     &  (5.d0*(singsig - tripsig))**2

       gc2(i,j) = (gsingsig(i,j) + 2.d0*gsingpi(i,j) + 
     &  3.d0*gtripsig(i,j) - 4.d0*gtrippi(i,j))/
     &  (5.d0*(singpi - trippi)) - (singsig + 2.d0*singpi + 
     &  3.d0*tripsig - 4.d0*trippi)*
     &  (5.d0*(gsingpi(i,j) - gtrippi(i,j)))/
     &  (5.d0*(singpi - trippi))**2  

       enddo
      enddo 

C     Calculate Cartesian gradients of triplet states
     ...
     &  (trippiy - trippix)
        ...
        do i=1,3    ! AJ ! RV
         do j=1,5
        gsing3bis(i,j,3,3) = gsing3(i,j,2,2) +
     &    (gsing3(i,j,3,3)-gsing3(i,j,2,2))*
     &    (trippiy - trippix) +
     &    (sing3(3,3)-sing3(2,2))*
     &    (gtrippiyth*gth(i,j) + gtrippiyphi*gphi(i,j) -
     &     gtrippixth*gth(i,j) - gtrippixphi*gphi(i,j))
         enddo
        enddo
...
C    Calculate gradient of triplet states

        do i=1,3
         do j=1,5

         gajsigma(i,j) = ((3.d0*gc2(i,j)*(sing3bis(1,1) - ech3) +
     &   3.d0*c2*(gsing3(i,j,1,1) - gech3(i,j)) - 4.d0*gc2(i,j)*
     &  (sing3bis(2,2) - ech3) - (4.d0*c2 - 2.d0)*(gsing3(i,j,2,2)
     &  - gech3(i,j)))* 
     & ( 3.d0*c2*(c1+1.d0) - (2.d0*c1 - 1.d0)*(4.d0*c2 - 2.d0) ) -
     &  (3.d0*c2*(sing3bis(1,1) - ech3)-(4.d0*c2 - 2.d0)*
     &  (sing3bis(2,2) - ech3))* (3.d0*gc2(i,j)*(c1+1.d0) +
     &   3.d0*c2*gc1(i,j) - 2.d0*gc1(i,j)*(4.d0*c2 - 2.d0) - 
     &   (2.d0*c1 - 1.d0)*4.d0*gc2(i,j)))/ 
     &   (3.d0*c2*(c1+1.d0) - (2.d0*c1 - 1.d0)*(4.d0*c2 - 2.d0))**2

        gajpi(i,j) = ((gc1(i,j)*(sing3bis(2,2) - ech3) + (1.d0+c1)*
     &   (gsing3(i,j,2,2) - gech3(i,j)) - 2.d0*gc1(i,j)*
     &  (sing3bis(1,1) - ech3) - (2.d0*c1 - 1.d0)*(gsing3(i,j,1,1) 
     &   - gech3(i,j)))*
     &  ( 3.d0*c2*(c1+1.d0) - (2.d0*c1 - 1.d0)*(4.d0*c2 - 2.d0) ) -
     &  ( (1.d0+c1)*(sing3bis(2,2) - ech3) - (2.d0*c1 - 1.d0)*
     &  (sing3bis(1,1) - ech3))*(3.d0*gc2(i,j)*(c1+1.d0) + 
     &   3.d0*c2*gc1(i,j) - 2.d0*gc1(i,j)*(4.d0*c2 -2.d0) - 
     &   (2.d0*c1 - 1.d0)*4.d0*gc2(i,j)))/
     &  (3.d0*c2*(c1+1.d0) - (2.d0*c1 - 1.d0)*(4.d0*c2 - 2.d0))**2  
        ...
        gtrip3a(i,j) = gsing3(i,j,1,1) - 2.d0*gajsigma(i,j)
         ...
         gajsigma(i,j) = ((3.d0*gc2(i,j)*(sing3bis(1,1) - ech3) +
     &   3.d0*c2*(gsing3(i,j,1,1) - gech3(i,j)) - 4.d0*gc2(i,j)*
     &  (sing3bis(3,3) - ech3) - (4.d0*c2 - 2.d0)*(gsing3bis(i,j,3,3)
     &  - gech3(i,j)))*
     & ( 3.d0*c2*(c1+1.d0) - (2.d0*c1 - 1.d0)*(4.d0*c2 - 2.d0) ) -
     &  (3.d0*c2*(sing3bis(1,1) - ech3)-(4.d0*c2 - 2.d0)*
     &  (sing3bis(3,3) - ech3))* (3.d0*gc2(i,j)*(c1+1.d0) +
     &   3.d0*c2*gc1(i,j) - 2.d0*gc1(i,j)*(4.d0*c2 - 2.d0) -
     &   (2.d0*c1 - 1.d0)*4.d0*gc2(i,j)))/
     &   (3.d0*c2*(c1+1.d0) - (2.d0*c1 - 1.d0)*(4.d0*c2 - 2.d0))**2

        gajpi(i,j) = ((gc1(i,j)*(sing3bis(3,3) - ech3) + (1.d0+c1)*
     &   (gsing3bis(i,j,3,3) - gech3(i,j)) - 2.d0*gc1(i,j)*
     &  (sing3bis(1,1) - ech3) - (2.d0*c1 - 1.d0)*(gsing3(i,j,1,1)    
     &   - gech3(i,j)))*
     &  ( 3.d0*c2*(c1+1.d0) - (2.d0*c1 - 1.d0)*(4.d0*c2 - 2.d0) ) -
     &  ( (1.d0+c1)*(sing3bis(3,3) - ech3) - (2.d0*c1 - 1.d0)*
     &  (sing3bis(1,1) - ech3))*(3.d0*gc2(i,j)*(c1+1.d0) +
     &   3.d0*c2*gc1(i,j) - 2.d0*gc1(i,j)*(4.d0*c2 -2.d0) - 
     &   (2.d0*c1 - 1.d0)*4.d0*gc2(i,j)))/ 
     &  (3.d0*c2*(c1+1.d0) - (2.d0*c1 - 1.d0)*(4.d0*c2 - 2.d0))**2

        gtrip3(i,j,3,3)= gsing3bis(i,j,3,3) - 2.d0*gajpi(i,j)
...
c      write(1,*) 'Fitted triplet gradients'
c      do i=1,mnat
c       write(1,*) "gE1(",i,") = ",gtrip3(1,i,1,1),gtrip3(2,i,1,1),
c     &                          gtrip3(3,i,1,1)," hartree/bohr"
c      enddo
c       write(1,*)

c print x,y,z components of the gradient for each atom for surface 2
c      do i=1,mnat
c       write(1,*) ,"gE2(",i,") = ",gtrip3(1,i,2,2),gtrip3(2,i,2,2),
c     &                          gtrip3(3,i,2,2)," hartree/bohr"
c      enddo
c       write(1,*)

c print x,y,z components of the gradient for each atom for surface 3
c      do i=1,mnat
c        write(1,*) ,"gE3(",i,") = ",gtrip3(1,i,3,3),gtrip3(2,i,3,3),
c     &                          gtrip3(3,i,3,3)," hartree/bohr"
c      enddo
...
c        gpemd(i,j,2,2) = gsing3(i,j,2,2)
c        gpemd(i,j,3,3) = gsing3(i,j,3,3)
c        gpemd(i,j,4,4) = 0.d0
c        gpemd(i,j,5,5) = 0.d0
c        gpemd(i,j,6,6) = 0.d0
c       enddo
c      enddo
...
C      Diabatic energies
        ...
        lambda(1,1) = cmplx(sing3(1,1)   , 0.d0)
        lambda(2,2) = cmplx(trip3(2,2)   ,0.d0)
        lambda(3,3) = cmplx(trip3(3,3)  ,0.d0)
        lambda(4,4) = cmplx(sing3(2,2)  ,0.d0)
        lambda(5,5) = cmplx(trip3(1,1)  ,0.d0)
        lambda(6,6) = cmplx(trip3(3,3) ,0.d0)
        lambda(7,7) = cmplx(sing3(3,3) ,0.d0)
        lambda(8,8) = cmplx(trip3(1,1) ,0.d0)
        lambda(9,9) = cmplx(trip3(2,2) ,0.d0)
        lambda(10,10) = cmplx(trip3(1,1) ,0.d0)
        lambda(11,11) = cmplx(trip3(2,2) ,0.d0)
        lambda(12,12) = cmplx(trip3(3,3) ,0.d0)
...
C      Diabatic gradients
      ...
      do i=1,3
       do j=1,5
         ...
         glambda(i,j,1,1) = cmplx(gsing3(i,j,1,1),0.d0)
         glambda(i,j,2,2) = cmplx(gtrip3(i,j,2,2),0.d0)
         glambda(i,j,3,3) = cmplx(gtrip3(i,j,3,3),0.d0)
         glambda(i,j,4,4) = cmplx(gsing3(i,j,2,2),0.d0)
         glambda(i,j,5,5) = cmplx(gtrip3(i,j,1,1),0.d0)
         glambda(i,j,6,6) = cmplx(gtrip3(i,j,3,3),0.d0)
         glambda(i,j,7,7) = cmplx(gsing3(i,j,3,3),0.d0)
         glambda(i,j,8,8) = cmplx(gtrip3(i,j,1,1),0.d0)
         glambda(i,j,9,9) = cmplx(gtrip3(i,j,2,2),0.d0)
         glambda(i,j,10,10) = cmplx(gtrip3(i,j,1,1),0.d0)
         glambda(i,j,11,11) = cmplx(gtrip3(i,j,2,2),0.d0)
         glambda(i,j,12,12) = cmplx(gtrip3(i,j,3,3),0.d0)
...
C     Apply the same transformation to the spin-free gradients
     ...
     &    glambda(l,m,k,j)
      ...
      do j=1,12
       do i=1,12
        do k=1,12
          gomega(l,m,i,j) = gomega(l,m,i,j)+ginterm(l,m,i,k)*SOC(k,j)
        enddo
       enddo
      enddo
      enddo
...
C     Add SOC constant to diagonal elements of diabatic spin-coupled matrix
      ...
      omega(1,1) = omega(1,1) - cmplx(delta, 0.d0)
      omega(2,2) = omega(2,2) - cmplx(delta, 0.d0)
      omega(3,3) = omega(3,3) + cmplx(2.d0*delta, 0.d0)
      omega(4,4) = omega(4,4) - cmplx(delta, 0.d0)
      omega(5,5) = omega(5,5) + cmplx(2.d0*delta, 0.d0)
      omega(6,6) = omega(6,6) - cmplx(delta, 0.d0)
      omega(7,7) = omega(7,7) - cmplx(delta, 0.d0)
      omega(8,8) = omega(8,8) + cmplx(2.d0*delta, 0.d0)
      omega(9,9) = omega(9,9) - cmplx(delta, 0.d0)
      omega(10,10) = omega(10,10) + cmplx(2.d0*delta, 0.d0)
      omega(11,11) = omega(11,11) - cmplx(delta, 0.d0)
      omega(12,12) = omega(12,12) - cmplx(delta, 0.d0)
      ...
      do j=1,12
       do i=1,12
          gpemd(l,m,i,j) = dble(gomega(l,m,i,j))
       enddo
      enddo
      enddo
      enddo
...
c       do i=1,12
c        do j=1,12
c         pemd(i,j) = lambda(i,j)
c      do k=1,3
c       do l=1,5
c         gpemd(k,l,i,j)=glambda(k,l,i,j)
c       enddo
c      enddo
c        enddo
c       enddo
...
c local
      ...
      integer it1,it2
      character*12 tname(mnc),tnameB(mnc),tnam
...
c         call Molpro 2006
c          call system_clock(it1)
          call dd_m06(symb,tnam,x,y,z,ptmp,gptmp,nat,
     &                                     mnat,ntmp,mnsurf,ifail)
c          call system_clock(it2)
...
c           save failure output
            call system('echo "IFAIL > 0 in DD" >> failures.out')
            call system('cat qc.out >> failures.out')
...
c           everything OK
          elseif (ifail.eq.1000) then
            call system('echo "IFAIL > 0 in DD1" >> failures.out')
            call system('cat qc.out >> failures.out')
c           retry with numerical gradient in backup template
            ifail=0
            do i=1,mnsurf
